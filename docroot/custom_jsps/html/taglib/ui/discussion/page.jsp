						...
						<div class="lfr-discussion <%= cssClass %>">
							...
							<aui:column cssClass="lfr-discussion-body">
								...
								<div class="lfr-discussion-message">
									...
									//---Custom code for creating links for Mentions---

									msgBody = linkMentions(message, msgBody, themeDisplay);
...
private String linkMentions(MBMessage message, String msgBody, ThemeDisplay themeDisplay) {
	Matcher matcher = _pattern.matcher(msgBody);

	String mentionedUsers = GetterUtil.getString(message.getExpandoBridge().getAttribute("mentionedUsers"));
	...
	while (matcher.find()) {
		...
		if (mentionedUsers.contains(result)) {
			try {
				User mentionedUser = UserLocalServiceUtil.getUserByScreenName(themeDisplay.getCompanyId(), result);

				String startLink = "<a href=\"" + mentionedUser.getDisplayURL(themeDisplay) + "\">";
				String endLink = "</a>";

				int pos = (matcher.start(1) - 1) + offset;

				msgBody = StringUtil.insert(msgBody, startLink, pos);

				offset += startLink.length();

				pos = matcher.end(1) + offset;

				msgBody = StringUtil.insert(msgBody, endLink, pos);

				offset += endLink.length();
			}
			catch (Exception e) {
			}
		}
